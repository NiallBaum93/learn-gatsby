...
exports.transformInlineFragments = void 0;
...
function transformField({
  ...
  const fieldType = typeMap.get((0, _helpers.findNamedTypeName)(field.type)) || {};
  const ofType = typeMap.get((0, _helpers.findNamedTypeName)(fieldType.ofType)) || {};
  if (fieldType.kind === `SCALAR` || fieldType.kind === `ENUM` || fieldType.kind === `NON_NULL` && ofType.kind === `SCALAR` || fieldType.kind === `LIST` && fieldType.ofType.kind === `SCALAR` ||
  ...
  if (fieldType.kind === `LIST` && isListOfGatsbyNodes && !isListOfMediaItems && hasIdField) {
    ...
    const listOfType = typeMap.get((0, _helpers.findNamedTypeName)(fieldType));
  ...
  const typeInfo = typeMap.get((0, _helpers.findNamedTypeName)(fieldType));
  const {
    fields
  } = typeInfo || {};
...
const createFragment = ({
  ...
  const typeName = (0, _helpers.findNamedTypeName)(type);
  if (buildingFragment) {
  ...
  const fragmentFields = fields.reduce((fragmentFields, field) => {
    ...
    const fieldTypeName = (0, _helpers.findNamedTypeName)(field.type);
    ...
    fieldType.name !== typeName && fieldType !== null && fieldType !== void 0 && (_fieldType$fields2 = fieldType.fields) !== null && _fieldType$fields2 !== void 0 && _fieldType$fields2.find(innerFieldField => (0, _helpers.findNamedTypeName)(innerFieldField.type) === typeName)) {
    ...
    if ((0, _helpers.findNamedTypeName)(field.type) !== typeName && !!transformedField) {
...
const transformFields = ({
  ...
  field,
  parentType
}) && !(0, _isExcluded.fieldIsExcludedOnAll)({
  pluginOptions,
  field
}) && !(0, _isExcluded.typeIsExcluded)({
  pluginOptions,
  typeName: (0, _helpers.findNamedTypeName)(field.type)
})).map(field => {
  const transformedField = transformField({
    maxDepth: queryDepth,
    gatsbyNodesInfo,
  ...
  const typeName = (0, _helpers.findNamedTypeName)(field.type);
  const fragment = fragments === null || fragments === void 0 ? void 0 : fragments[typeName];
  ...
  if (fragment && transformedField && buildingFragment !== typeName) {
    ...
    if (transformedField !== null && transformedField !== void 0 && (_transformedField$inl = transformedField.inlineFragments) !== null && _transformedField$inl !== void 0 && _transformedField$inl.length) {
      transformedField.inlineFragments = transformedField.inlineFragments.filter(fieldInlineFragment =>
...
const recursivelyTransformFields = ({
  ...
  const typeName = (0, _helpers.findNamedTypeName)(parentType);
